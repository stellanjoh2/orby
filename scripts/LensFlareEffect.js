...
const DEFAULT_DISC_GLOW_COLOR = '#ff8844';
...
const fragmentShader = `
    ...
    vec3 drawflare(vec2 p, float intensity, float rnd, float speed, int id)
    {
        ...
        if(flareSpeed > 0.0){
            ...
            return vec3(comp) * (vec3(1.0) * expgrad * intensity);
        }
    }
    ...
    vec3 drawDiscGlow(vec2 uv, vec2 pos) {
        ...
        float radius = sizeNorm * 0.42;

        float intensity = exp(-pow(dist / max(radius, 0.0001), 1.35));

        float brightness = 1.15 * (uDiscGlowIntensity * 0.1);
        return uDiscGlowColor * intensity * vis * brightness;
    }
    ...
    void main()
    {
        ...
        if (uOccluded && opacity < 0.01 && !showSunDisc && !showDiscGlow) {
            gl_FragColor = vec4(0.0);
            return;
        }
...
const createPlaceholderTexture = () => {
  ...
  if ('colorSpace' in texture && THREE.LinearSRGBColorSpace) {
    texture.colorSpace = THREE.LinearSRGBColorSpace;
  }
  return texture;
};

export class LensFlareEffect extends THREE.Mesh {
  constructor(options = {}) {
    ...
    const uniforms = {
      ...
      colorGain: { value: new THREE.Vector3(95, 12, 10) },
      starPoints: { value: options.starPoints ?? 5 },
      glareSize: { value: options.glareSize ?? 0.55 },
      flareSize: { value: options.flareSize ?? 0.004 },
      flareSpeed: { value: options.flareSpeed ?? 0.4 },
      flareShape: { value: options.flareShape ?? 1.2 },
      haloScale: { value: options.haloScale ?? 0.5 },
      opacity: { value: options.opacity ?? 0.8 },
      animated: { value: options.animated ?? true },
      anamorphic: { value: options.anamorphic ?? false },
      secondaryGhosts: { value: options.secondaryGhosts ?? true },
      starBurst: { value: options.starBurst ?? true },
      ghostScale: { value: options.ghostScale ?? 0.3 },
      aditionalStreaks: { value: options.aditionalStreaks ?? true },
      followMouse: { value: options.followMouse ?? false },
      lensDirtTexture: { value: createPlaceholderTexture() },
      uDispersal: { value: 0.3 },
    ...
    this.discGlowColorHex = options.discGlowColor ?? DEFAULT_DISC_GLOW_COLOR;
    ...
    loader.load(
      options.dirtTexture ?? DEFAULT_DIRT_TEXTURE,
      (texture) => {
        if ('colorSpace' in texture && THREE.LinearSRGBColorSpace) {
          texture.colorSpace = THREE.LinearSRGBColorSpace;
        }
        ...
        this.material.uniforms.lensDirtTexture.value = texture;
      },
      undefined,
      () => {
        console.warn('LensFlareEffect: Failed to load dirt texture.');
      },
    );

    this.material.onBeforeRender = (renderer, scene, camera) => {
      this.prepareFrame(renderer, scene, camera);
    };

    this.setColor(this.colorHex);
    this.setSunDiscColor(this.sunDiscColorHex);
    this.setDiscGlowColor(this.discGlowColorHex);
  ...
  updateProjectedPosition(camera, scene) {
    ...
    // Throttle occlusion ray tests only — projection still updates every frame.
  ...
  updateAnchorPosition() {
    const azimuthRad = THREE.MathUtils.degToRad(this.azimuthDeg ?? 0);
    const elevationRad = THREE.MathUtils.degToRad(this.elevationDeg ?? 0);
    const horizontalRadius =
      Math.max(0.001, Math.cos(elevationRad)) * this.distance;
    const y = Math.sin(elevationRad) * this.distance;

    this.anchorWorld.set(0, y, -horizontalRadius);
    this.anchorWorld.applyAxisAngle(yAxis, azimuthRad);
  }
  ...
  setColor(hex) {
    ...
    this.colorHex = hex;
    const uniformColor = this.material.uniforms.colorGain.value;
    uniformColor.set(
      color.r * 255,
      color.g * 255,
      color.b * 255,
  ...
  setDiscGlowIntensity(value) {
    if (!Number.isFinite(value)) return;
    const clamped = Math.max(0, Math.min(10, value));
    this.material.uniforms.uDiscGlowIntensity.value = clamped;
  }

  setDiscGlowSize(value) {
    if (!Number.isFinite(value)) return;
    const clamped = Math.max(0, Math.min(10, value));
    this.material.uniforms.uDiscGlowSize.value = clamped;
  }

  setDiscGlowColor(hex) {
    if (!hex) return;
    let color;
    try {
      color = new THREE.Color(hex);
    ...
    this.discGlowColorHex = hex;
    this.material.uniforms.uDiscGlowColor.value.set(color.r, color.g, color.b);
  }
