...
/**
 * Screen-space Apple II HGR — 280×192 macro pixels, NTSC artifact palette.
 */
export class CreativeLookApple2Pass {
  /** @param {import('three').WebGLRenderer} renderer */
  constructor(renderer) {
    this.renderer = renderer;
    this._pixelRatio = Math.max(1, renderer?.getPixelRatio?.() ?? 1);

    const cell = creativeApple2CellSize();
    this.material = new THREE.ShaderMaterial({
      uniforms: {
        tDiffuse: { value: null },
        uResolution: { value: new THREE.Vector2(1, 1) },
        uCellSize: { value: new THREE.Vector2(cell.width, cell.height) },
        uBgColor: { value: new THREE.Color(A2_BG_HEX) },
        uMasterHue: { value: 0 },
      },
      vertexShader: VERTEX_SHADER,
      fragmentShader: A2_POST_FRAGMENT,
      depthTest: false,
      depthWrite: false,
    });
  ...
  pinReferenceLogicalSize(_logicalW, _logicalH) {
    this._referenceLogicalSize.set(A2_REF_LOGICAL_WIDTH, A2_REF_LOGICAL_HEIGHT);
