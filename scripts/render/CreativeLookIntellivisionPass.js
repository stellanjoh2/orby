...
/**
 * Screen-space Intellivision crush — 160×96 macro pixels, STIC 16-color palette.
 */
export class CreativeLookIntellivisionPass {
  /** @param {import('three').WebGLRenderer} renderer */
  constructor(renderer) {
    this.renderer = renderer;
    this._pixelRatio = Math.max(1, renderer?.getPixelRatio?.() ?? 1);

    const cell = creativeIntellivisionCellSize();
    this.material = new THREE.ShaderMaterial({
      uniforms: {
        tDiffuse: { value: null },
        uResolution: { value: new THREE.Vector2(1, 1) },
        uCellSize: { value: new THREE.Vector2(cell.width, cell.height) },
        uBgColor: { value: new THREE.Color(INTV_BG_HEX) },
        uMasterHue: { value: 0 },
      },
      vertexShader: VERTEX_SHADER,
      fragmentShader: INTV_POST_FRAGMENT,
      depthTest: false,
      depthWrite: false,
    });
  ...
  pinReferenceLogicalSize(_logicalW, _logicalH) {
    this._referenceLogicalSize.set(INTV_REF_LOGICAL_WIDTH, INTV_REF_LOGICAL_HEIGHT);
