...
import { GradingShader } from '../shaders/index.js';
import {
...
/**
 * Exposure → color adjust → tone map (+ vignette) in one fullscreen pass.
 */
export class GradingController {
  constructor(renderer) {
    this.pass = new ShaderPass(GradingShader);
  ...
  reset() {
    ...
    if (this.uniforms.exposure) {
      this.uniforms.exposure.value = 1.0;
    }
    if (this.uniforms.toneMappingType) {
      this.uniforms.toneMappingType.value = 4;
    }
    if (this.uniforms.vignetteIntensity) {
      this.uniforms.vignetteIntensity.value = 0.0;
    }
    if (this.uniforms.vignetteColor) {
      this.uniforms.vignetteColor.value.set('#080808');
    }
  ...
  setExposure(value) {
    if (!this.uniforms.exposure) return;
    this.uniforms.exposure.value = value;
  }

  setToneMapping(value) {
    if (value === 'linear') value = 'none';
    const toneMappingValue = TONE_MAPPING_MAP[value] ?? 4;
    if (this.uniforms.toneMappingType) {
  ...
  setVignetteColor(color) {
    if (this.uniforms.vignetteColor) {
      this.uniforms.vignetteColor.value = new THREE.Color(color);
    }
  }
